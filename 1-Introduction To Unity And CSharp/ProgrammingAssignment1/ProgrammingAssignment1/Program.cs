    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (input[0] != 'q')
            {
                ...
                float ilkHesaplama = (deltaX * deltaX) + (deltaY * deltaY);
                float distance =(float) Math.Sqrt(ilkHesaplama);
                
                float angleFromX = (float)Math.Atan2(deltaY, deltaX);
                float degrees = (180 / (float)Math.PI) * angleFromX;

                Console.WriteLine(distance + " " + degrees);
